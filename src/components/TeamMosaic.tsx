...
function Tile({ src, size = "md", i = 0 }: { src: string; size?: "sm" | "md" | "lg"; i?: number }) {
  ...
  return (
    <Reveal delay={i * 80} className={`relative w-full ${dims} overflow-hidden rounded-2xl bg-surface`}>
      <Image
        src={`/images/${src}.jpg`}
        alt=""
        fill
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8 grid lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] gap-12 items-start">
        <div className="lg:sticky lg:top-28">
          <Reveal as="h2" className="text-4xl md:text-6xl font-bold tracking-tight leading-[1.05]">
            We <span className="gradient-text">create</span> your super team
          </Reveal>
          <Reveal delay={120} as="div" className="mt-8 space-y-3 text-xl md:text-2xl text-white/70 leading-snug">
            <p>We identify</p>
            <p>select</p>
            <p>and attract <span className="text-white">specialized talent</span></p>
            <p>in the pharma industry</p>
            <p>all over Latin America.</p>
          </Reveal>
          <Reveal delay={240} className="mt-8">
            <a
              href="#contact"
              className="inline-flex items-center gap-2 rounded-full bg-white text-ink px-6 py-3 text-sm font-medium hover:bg-accent transition"
            >
              Start hiring with Hubi →
            </a>
          </Reveal>
        </div>
